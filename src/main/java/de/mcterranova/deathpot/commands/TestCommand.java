...
public class TestCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (p.getTargetBlockExact(20) == null || !(p.getTargetBlockExact(20).getState() instanceof DecoratedPot pot))
            return false;
        p.sendMessage("Item: " + Arrays.toString(pot.getPersistentDataContainer().get(itemKey, DataType.ITEM_STACK_ARRAY)));
        p.sendMessage("User: " + pot.getPersistentDataContainer().get(userKey, de.mcterranova.terranovaLib.persistentData.terraDataType.UUID));
        p.sendMessage("Time: " + pot.getPersistentDataContainer().get(timeKey, de.mcterranova.terranovaLib.persistentData.terraDataType.Instant));
        p.sendMessage("UUID: " + pot.getPersistentDataContainer().get(uuidKey, de.mcterranova.terranovaLib.persistentData.terraDataType.UUID));

        PersistentDataContainer pdc = p.getItemInHand().getItemMeta().getPersistentDataContainer();
        p.sendMessage("PDC: " + pdc.get(dumm, de.mcterranova.terranovaLib.persistentData.terraDataType.UUID));
        return false;
